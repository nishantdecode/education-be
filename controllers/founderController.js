...
const updateFounder = async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      founder: founderToUpdate,
    });
  } catch (error) {
    console.error("Error updating founder:", error);
    res
      .status(500)
      .json({ message: "Error updating founder", error: error.message });
  }
};
...
const getAllFounders = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "All founders retrieved successfully", founders });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error retrieving founders", error: error.message });
  }
};
