  ...
  try {
    ...
    if (ratings && ratings.length > 0) {
      const ratingFilters = ratings.map((r) => {
        ...
        return sequelize.where(
          sequelize.cast(sequelize.json("data.ratings"), "FLOAT"),
          {
            [Op.gte]: rating,
            [Op.lt]: rating + 1,
          }
    ...
    if (minFees) {
      filters.where[Op.and].push(
        sequelize.where(
          sequelize.cast(sequelize.json("data.fees.min"), "INTEGER"),
          Op.gte,
          minFees
        )
      );
    ...
    if (maxFees) {
      filters.where[Op.and].push(
        sequelize.where(
          sequelize.cast(sequelize.json("data.fees.max"), "INTEGER"),
          Op.lte,
          maxFees
        )
      );
    ...
    if (minSeats) {
      filters.where[Op.and].push(
        sequelize.where(
          sequelize.cast(sequelize.json("data.seats"), "INTEGER"),
          Op.gte,
          minSeats
        )
      );
    ...
    const { rows: coachings, count: totalCount } =
      await Coaching.findAndCountAll({
        ...filters,
        offset: (page - 1) * show,
        limit: show,
      });
...
const getFilterData = async (req, res) => {
  try {
    ...
    const locationsData = await Coaching.findAll({
      attributes: [
        [
          sequelize.fn("DISTINCT", sequelize.literal("data->>'location'")),
          "location",
        ],
      ],
      raw: true,
    ...
    const specializationData = await Coaching.findAll({
      attributes: [
        [
          sequelize.fn("DISTINCT", sequelize.literal("data->>'specialization'")),
          "specialization",
        ],
      ],
      raw: true,
    });
    const specializationList = specializationData.map((item) => item.specialization);
    ...
    const minFeesData = await Coaching.findAll({
      attributes: [
        [
          sequelize.fn(
            "MIN",
            sequelize.cast(sequelize.literal("data->'fees'->>'min'"), "INTEGER")
          ),
          "minFees",
        ],
      ],
    ...
    const maxFeesData = await Coaching.findAll({
      attributes: [
        [
          sequelize.fn(
            "MAX",
            sequelize.cast(sequelize.literal("data->'fees'->>'max'"), "INTEGER")
          ),
          "maxFees",
        ],
      ],
