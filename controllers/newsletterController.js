...
const apply = async (req, res) => {
  ...
  try {
    ...
    sendMail(
      email,
      "Subscribed",
      "Dear User, You have successfully subscribed for Edmertion newsletter. You will get regular updates in your inbox."
    );
...
const applications = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "All submissions retrieved successfully", submissions });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error retrieving submissions", error: error.message });
  }
};
