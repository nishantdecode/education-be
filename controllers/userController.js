...
const generateUserId = (stateAcronym, age, sequenceNo, nameInitials) => {
  return `${stateAcronym}${age.toString().padStart(2, "0")}${sequenceNo
    .toString()
...
const sendOtp = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "OTP sent successfully", emailExists: true });
  } catch (error) {
...
const getUserDataByUserId = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "User data retrieved successfully", user, blogs });
  } catch (error) {
    res
      .status(500)
...
const getUserDataWithInfoPercentage = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message: "Error retrieving user data with information percentage",
      error: error.message,
    });
  }
};
...
const getUsersCountByCountry = async (req, res) => {
  try {
    const usersByCountry = await User.findAll({
      attributes: [
        "country",
        [sequelize.fn("COUNT", sequelize.col("country")), "userCount"],
      ],
      group: ["country"],
    });

    res.status(200).json({
      message: "User count by country retrieved successfully",
      usersByCountry,
    });
  } catch (error) {
    res.status(500).json({
      message: "Error retrieving user count by country",
      error: error.message,
    });
  }
};
...
const requestVerificationTag = async (req, res) => {
  ...
  try {
    ...
    if (existingRequest) {
      return res
        .status(409)
        .json({ message: "Verification request already submitted" });
    }
...
const getUserTagsAndBlogs = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message: "Error retrieving user tags and blogs",
      error: error.message,
    });
  }
};
